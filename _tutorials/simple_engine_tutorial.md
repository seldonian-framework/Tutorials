...
<div class="container p-3 my-5 border" style="background-color: #f3f4fc;">
    ...
    <ol>
        ...
        <li> Define an initial solution function which takes the features and labels as inputs and outputs an initial weight vector to start candidate selection. In this case we will define a function {% ihighlight python %} initial_solution() {% endihighlight %} function which just returns a zero vector as the initial solution.</li>
...
{% endhighlight %}
    ...
    <p>
    Notice in the last few lines of the script that {% ihighlight python %} SA.run() {% endihighlight %} returns two values. {% ihighlight python %} passed_safety {% endihighlight %} is a boolean indicating whether the candidate solution found during candidate selection passed the safety test. If {% ihighlight python %}passed_safety==False {% endihighlight %}, then {% ihighlight python %} solution="NSF" {% endihighlight %}, i.e. "No Solution Found". If {% ihighlight python %} passed_safety==True {% endihighlight %} then the solution is the array of model weights that cause the safety test to be passed. In this example, you should get {% ihighlight python %} passed_safety=True {% endihighlight %} and a candidate solution of something like: {% ihighlight python %} [0.16911355 0.1738146] {% endihighlight %}, although the exact numbers might differ slightly depending on your machine's random number generator.
</p>
<p> Also notice that {% ihighlight python %} SA.run() {% endihighlight %} does not return what the value of the primary objective actually was on the safety test. Given that it passed the safety test, we know that it must satisfy: $1.25 \leq {\theta} \leq 2.0$ (with high probability). The {% ihighlight python %} SA {% endihighlight %} object provides the introspection we need to  extract this information:
