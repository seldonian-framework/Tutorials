...
<p>
  ...
  <div align="center">
    <figure class='mt-4'>
        ...
        <figcaption class="figure-caption"><b>Figure 3</b> - Bounding the model $\hat{y}=2x$ using the sigmoid function. The bounded model closely matches the unbounded model in the middle of the domain. </figcaption>
    </figure> 
    </div>
...
<li>The last method we need here is <code class='highlight'>calculate_value()</code>, which calculates not the bound but the actual value of the CVaR statistic. This is not necessary for the Seldonian algorithm to run, but it is used in the Experiment framework for making the failure rate plot. This is only implemented for continuous probability distributions. Here is our implementation of this method:
{% highlight python %}
...
<p>
  We can now run this Seldonian algorithm using the script below. Most of the script script is boilerplate code for running the Seldonian Engine, but we want to note a few things first:
</p>
<ul>
...
<p> 
If we save this script to a file called <code>run_engine_custom_base_node.py</code> via the command line like:
{% highlight bash %}
